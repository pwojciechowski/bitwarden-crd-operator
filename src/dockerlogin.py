...
from utils.utils import (
    ...
    should_skip_due_to_auth_cooldown,
    delete_secret,
    create_secret,
    update_secret,
)
...
def create_dockerlogin(
    logger, secret, secret_json, username_ref, password_ref, registry
):
    ...
    cred_field = str(
        base64.b64encode(f"{_username}:{_password}".encode("utf-8")), "utf-8"
    )
    ...
    secret.data[".dockerconfigjson"] = str(
        base64.b64encode(json.dumps(auths_dict).encode("utf-8")), "utf-8"
    )
    return secret
...
def create_managed_registry_secret(spec, name, namespace, logger, **kwargs):
    ...
    secret = create_dockerlogin(
        logger, secret, secret_json_object["data"], username_ref, password_ref, registry
    )
...
@kopf.timer(
    "registry-credential.lerentis.uploadfilter24.eu",
    interval=bw_sync_interval,
    initial_delay=10,
)
def update_managed_registry_secret(
    spec, status, name, namespace, logger, body, **kwargs
):
    username_ref = spec.get("usernameRef")
    password_ref = spec.get("passwordRef")
    registry = spec.get("registry")
    ...
    old_config, old_secret_name, old_secret_namespace, old_secret_type = (
        parse_old_config(body)
    )
    ...
    try:
        ...
        if should_skip_due_to_auth_cooldown(exc):
            logger.info(str(exc))
            return
        raise
    logger.info(f"Locking up secret with ID: {bw_id}")
